...
const OutstandingLoansList = () => {
  ...
  const getLoansApprovedData = async () => {
    ...
    try {
      const result = await axios.get(
        `${process.env.REACT_APP_URL}/loans/approved?page=${pages.currentPage}`,
        {
          headers: {
  ...
  const onRowClicked = () =>
    TransactionQueue.map(
      (result) => (e) =>
        history.push("/outstandingLoanDetails?id=" + result._id)
    );
      ...
      format: (row) => (
        <div data-tag="allowRowEvents" className="tq-td">
          <div
            aria-hidden="true"
            onClick={(e) =>
              history.push("/outstandingLoanDetails?id=" + row.id)
            }
          >
            {row.id}
          </div>
    ...
    <div>
      ...
      >
        ...
        <NavLink to="/outstandingLoans">
          <ListItemText primary={"Outstanding Loans"} />
        </NavLink>
      </div>
      {isLoading && isLoading ? (
        ""
